...
def load_df(user, pw, db='kiva', table='loans', host='localhost', port='5432',
            dates=['2012-04-01', '2015-04-01'], cols = '*'):
    '''
    Input postgresql username & pw
    Returns pandas df of kiva loan data
    '''
    pipe = data_pipeline.Pipeline()
    pipe.setup_sql(user, pw, db=db, host=host, port=port)
    pipe.load_from_sql(cols=cols, date_range=dates, table=table)
    return pipe.df
...
def competing_loans(df):
    ...
    bar_width = 0.4
    plt.bar(index, men_rate, bar_width, alpha=.8, color='b', label='Men')
    plt.bar(index + bar_width, women_rate, bar_width, alpha=.6, color='r',
            label='Women')
    ...
    plt.xticks(index + bar_width, interval)
    plt.legend()
